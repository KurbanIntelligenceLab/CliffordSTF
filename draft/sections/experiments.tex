...
Force cosine similarity isolates directional fidelity from magnitude accuracy and is standard on OC20 but has not been previously reported on MD17; we include it as our primary mechanistic diagnostic. The three invariant architectures (SchNet, DimeNet++, FAENet) compute forces via energy gradients whose cosine values depend on energy--force loss balancing, so we include them in tables for completeness but exclude them from the directional narrative; GotenNet did not converge on MD17 under our uniform protocol---its published recipe matches our learning rate ($10^{-4}$) but additionally relies on a $10{,}000$-step LR warmup, an on-plateau scheduler, and early-stopping patience $150$, none of which are in our fixed-budget protocol; we report its converged-by-our-criteria value of $0.030$ in Table~\ref{tab:md17_fcos} and exclude it from the per-baseline directional narrative. EquiformerV2 predicts absolute (non-referenced) molecular energies rather than energy differences, so its MD17 energy MAE reflects absolute binding-energy magnitudes ($\sim$247\,eV, consistent across all three $L_{\max}$ settings) rather than prediction error; we mark this with $\dagger$ in the table and it is not comparable to the energy-referenced MAEs of other models.
