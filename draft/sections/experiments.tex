...
We evaluate on five benchmark families: \textbf{rMD17} (10 small-molecule S2EF trajectories; 900/100/1000 per molecule), \textbf{QM9} (six scalar targets), \textbf{Molecule3D} (large-scale HOMO--LUMO gap), \textbf{OC20} (200k subset, IS2RE and S2EF), and \textbf{OC22} (full IS2RE-Total and S2EF-Total). We compare against twelve architecture families spanning invariant message-passing (SchNet~\citep{schutt2017schnet}, DimeNet++~\citep{gasteiger2020dimenetpp}), vector equivariant (PaiNN~\citep{schutt2021painn}, TorchMD-NET~\citep{tholke2022torchmdnet}, ViSNet~\citep{wang2024visnet}), tensor-field (NequIP~\citep{batzner2022nequip}, EquiformerV2~\citep{liao2024equiformerv2}, GotenNet~\citep{aykent2025gotennet}), body-ordered (MACE~\citep{batatia2022mace}, ICTP~\citep{zaverkin2024ictp}), frame-averaging (FAENet~\citep{duval2023faenet}), and Clifford-algebraic (our base $\mathrm{Cl}(3,0)$, recovered from CliffordSTF by disabling all extensions); MACE, ICTP, and EquiformerV2 are additionally run at $L_{\max} \in \{1,2,3\}$, $\{1,2,3\}$, and $\{2,3,4\}$ respectively where applicable to trace angular resolution within a family, giving up to eighteen baseline configurations on rMD17. All models use a unified training framework at approximately $10^6$ parameters, Adam with learning rate $10^{-4}$, no weight decay, early stopping at patience 30, and no EMA or warm-up; per-benchmark settings (epochs, batch size, loss, seeds) are in Supplementary~\ref{supp:protocol}, and per-benchmark model architecture hyperparameters (channels, layers, $L_{\max}$, RBF) are tabulated in Supplementary~\ref{supp:model_settings}. The protocol targets representational comparability at fixed capacity and does not chase per-architecture SOTA. Under this fixed-budget regime several baselines do not reach their published accuracies, which is expected and honestly reported in the tables; the relevant comparison is within-protocol, not across papers. Metrics are energy MAE (meV on rMD17/QM9/Molecule3D, eV on OC22/OC20), force MAE (meV/\AA{} on rMD17, eV/\AA{} on OC22/OC20), and force cosine similarity (standard raw value in $[-1,1]$; higher is better). All numbers are mean $\pm$ standard deviation across seeds.

\subsection{Force directional accuracy on rMD17}\label{sec:results_md17}

Force cosine similarity isolates directional fidelity from magnitude accuracy and is standard on OC20 but has not been previously reported on rMD17; we include it as our primary mechanistic diagnostic. SchNet and DimeNet++ are invariant and compute forces via energy gradients whose cosine values depend on energy--force loss balancing; FAENet predicts forces directly from a learned head but uses stochastic frame averaging rather than architectural equivariance, so per-sample directional fidelity is not strictly defined; GotenNet is $L\!\geq\!2$ ($\mathtt{lmax}{=}2$) and predicts forces directly but did not converge on rMD17 under our uniform protocol---its published recipe matches our learning rate ($10^{-4}$) but additionally relies on a $10{,}000$-step LR warmup, an on-plateau scheduler, and early-stopping patience $150$, none of which are in our fixed-budget protocol. We include all four in Table~\ref{tab:md17_fcos} for completeness, mark FAENet and GotenNet with $\ast$ in their natural $L$-cohort rows, gray out the strictly invariant rows (SchNet, DimeNet++), and exclude all four from the per-architecture directional narrative below. EquiformerV2 predicts absolute (non-referenced) molecular energies rather than energy differences, so its rMD17 energy MAE reflects absolute binding-energy magnitudes ($\sim$247\,eV, consistent across all three $L_{\max}$ settings) rather than prediction error; we mark this with $\dagger$ in the table and it is not comparable to the energy-referenced MAEs of other models.
...
\begin{table}[t]
\centering
\caption{\textbf{rMD17 force cosine similarity} per molecule and aggregate (mean $\pm$ std across 3 seeds; higher is better, standard $[-1,1]$ range). Top block: $L\!\geq\!2$ direct-force models. Middle block: $L\!\leq\!1$ direct-force models. Bottom block (gray): strictly invariant models (SchNet, DimeNet++) whose forces come from energy gradients. $\dagger$ EquiformerV2 predicts absolute energies; see \S\ref{sec:results_md17}. $\ast$ FAENet predicts forces directly but uses stochastic frame-averaged equivariance; GotenNet is $L\!\geq\!2$ ($\mathtt{lmax}{=}2$) but did not converge under our fixed-budget protocol---both included in their natural $L$-cohort for completeness and excluded from the per-architecture directional narrative (see \S\ref{sec:results_md17}). Under our protocol, MACE and ICTP at every $L_{\max}$ attain force MAE of 3000--6000\,meV/\AA{} (full table in Supplementary~\ref{supp:md17_permol}), reflecting non-convergence at $10^6$ parameters rather than architecture quality at recommended hyperparameters.}
\label{tab:md17_fcos}
...
\textbf{OC22.} CliffordSTF is competitive across the OC22 benchmark. On IS2RE, it attains in-distribution MAE $4.09$\,eV, the best among $L\!\geq\!2$ architectures (next-best EquiformerV2 at $L{=}4$ attains $4.90$\,eV, and the base Clifford model $4.42$\,eV; PaiNN, an $L\!=\!1$ vector model, attains the lowest overall at $3.52$\,eV); on the OOD split it attains $6.46$\,eV, matching EquiformerV2 ($6.62$\,eV) within seed variance and behind GotenNet ($5.77$\,eV), which is the strongest IS2RE-OOD model in the study. CliffordSTF additionally attains the best $L\!\geq\!2$ EwT out-of-distribution ($0.31$ vs $0.23$ for EquiformerV2). On S2EF, CliffordSTF attains in-distribution energy MAE $2.87$\,eV, close to the best model in the study (base Clifford at $2.84$\,eV), and out-of-distribution energy MAE $5.08$\,eV, the best in the study (EquiformerV2 $L{=}4$ at $5.26$\,eV). On force metrics EquiformerV2 leads with cosine similarity $0.067$ and $0.062$ in- and out-of-distribution versus CliffordSTF at $0.050$; force MAEs for CliffordSTF, base Clifford, and EquiformerV2 cluster tightly within $0.001$--$0.002$\,eV/\AA{} of each other across splits, reflecting the difficulty of direct-force prediction on OC22 at this parameter scale. Full numbers are in Table~\ref{tab:oc22}.
