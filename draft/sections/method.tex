...
Expand $u_iv_j = \tfrac{1}{3}\delta_{ij}(u\cdot v) + \tfrac{1}{2}(u_iv_j - u_jv_i) + \mathrm{STF}_2(u,v)_{ij}$. The antisymmetric part equals $\tfrac{1}{2}\varepsilon_{ijk}(u\times v)_k$, and $u\times v = \star(u\wedge v)$ under the Hodge isomorphism $\Lambda^2\mathbb{R}^3 \cong \mathbb{R}^3$. Equivariance follows from $\mathrm{SO}(3)$-equivariance of the geometric product, the Hodge star, and symmetric/trace-removal operations. Full coordinates in Supplementary~\ref{supp:cg_proof}.
\end{proof}

Higher-order couplings ($\mathbf{2}\otimes\mathbf{1}$, $\mathbf{2}\otimes\mathbf{2}$, $\mathbf{3}\otimes\mathbf{1}$, with the $L{=}4$ component of $\mathbf{2}\otimes\mathbf{2}$ outside our $L_{\max}{=}3$ cut) are recovered by analogous STF contractions, enumerated with explicit coordinates in Supplementary~\ref{supp:cg_proof}. The cut is set at $L_{\max}{=}3$ to balance new $L{\geq}2$ capacity against the matched $10^6$-parameter budget; an STF$_4$ extension following the construction of \citet{shao2025orthonormal} is straightforward but unnecessary for closing the directional gap diagnosed in \S\ref{sec:limitation}. Each constructive map is a polynomial in its inputs with constant rational coefficients matching the classical detracing rule $1/(2\ell-1)$ in $d=3$. We verify each map against a direct Wigner $3j$-symbol reference to machine precision (Supplementary~\ref{supp:cg_test}), so the architectural realisation reproduces the orthonormal equivalence in the specific cases used by CliffordSTF.

\subsection{Architecture}\label{sec:architecture}

Figure~\ref{fig:architecture} summarises the pipeline. CliffordSTF extends the 8D multivector with STF channels: \textbf{Base} (8D, $2\cdot L{=}0 \oplus 2\cdot L{=}1$); \textbf{+STF$_2$} (13D); \textbf{+STF$_3$} (20D). Setting $\mathrm{stf\_mode}{=}\text{none}$ with Hodge, routing, and cross-track coupling all disabled recovers the base Clifford model bit-for-bit. A per-layer grade schedule activates grades progressively, yielding grade-sparse geometric products in early layers. The only nonstandard primitives are the $8\times8$ Cayley table and the closed-form STF operations of \S\ref{sec:stf_channels}; the rest of the model uses standard PyTorch operations.
...
\]
with grade-sparse GP dispatch and an invariant scalar attention weight $\alpha_{ij}$ from multi-head dot-product attention on grade-0 features with an additive RBF bias. STF$_2$ messages combine generation from grade-1 features ($\mathrm{STF}_2(h_j^{\mathrm{vec}}, \hat{r}_{ij})$) with radially-modulated propagation of the sender's STF$_2$ features; STF$_3$ messages are analogous, generated via $\mathrm{STF}_3(h_j^{\mathrm{STF}_2}, \hat{r}_{ij})$.
...
\textbf{Update.} Iterated augmented products provide multi-body correlations up to body order 4, followed by a block-diagonal CliffordSTFLinear, equivariant gating (SiLU on scalars, norm-gating on $L \geq 1$ \citep{weiler20183d}), a residual connection, and track-wise equivariant normalisation (Supplementary~\ref{supp:normalization}).
