...
def test_cli_main_uses_packaged_dimenetpp_baseline(tmp_path: Path) -> None:
    """Drive ``cli.main`` with the ``dimenetpp`` baseline.

    Same adapter pattern as SchNet (PyG-native, autograd forces).
    Heavy DimeNet++ knobs are shrunk via CLI dot-overrides for CPU speed.
    """
    data_root = tmp_path / "data"
    output_root = tmp_path / "outputs"
    yaml_path = _write_dataset_training_override_yaml(
